...
**Need more help?** Check the install script output for detailed logs.

## Adding New Game Domains

If you discover a game that doesn't work, you can add its domains manually:

```bash
# On your VPS
cd /root/doh
./scripts/maintenance/add-game-domain.sh domain.com
```

This will automatically:
- Add the domain to Smart DNS
- Configure SNIProxy
- Restart services

## Maintenance Scripts

The `scripts/maintenance/` folder contains tools for ongoing maintenance:

- **`add-game-domain.sh`** - Add individual game domains when needed
- **`fix-xbox-connectivity.sh`** - Troubleshoot Xbox connection issues

These are optional tools you use after installation if needed.

## Files Included

- `docker-compose.yml` - Docker configuration (generated by installer)
- `coredns/` - Smart DNS settings (generated by installer)
- `scripts/setup/install.sh` - Main installer (does everything!)
- `scripts/setup/setup-letsencrypt.sh` - SSL certificate setup
- `scripts/maintenance/` - Maintenance tools (optional)
